            ...
            <div className="bg-white p-4 rounded-2xl border border-outline-variant/20 shadow-sm">
              ...
              <form onSubmit={handleEditPatient} className="space-y-2.5">
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div><label className="text-[8px] font-bold text-outline block mb-0.5">FECHA NACIMIENTO</label><input type="date" value={editingPatient.fechaNacimiento || ''} onChange={(e) => setEditingPatient({ ...editingPatient, fechaNacimiento: e.target.value })} className="w-full bg-surface-container-low border border-outline-variant/30 rounded-lg p-2.5 text-xs outline-none focus:ring-1 focus:ring-primary font-semibold" /></div>
                </div>
                ...
                <div className="border-t border-outline-variant/15 pt-2.5">
                  ...
                  <div className="space-y-2">
                    <div><label className="text-[8px] font-bold text-outline block mb-0.5">NOMBRE ACUDIENTE</label><input type="text" value={editingPatient.acudienteNombre || ''} onChange={(e) => setEditingPatient({ ...editingPatient, acudienteNombre: e.target.value })} className="w-full bg-surface-container-low border border-outline-variant/30 rounded-lg p-2.5 text-xs outline-none focus:ring-1 focus:ring-primary font-semibold" placeholder="e.g. Carlos Pérez" /></div>
                    <div className="grid grid-cols-2 gap-2">
                      <div><label className="text-[8px] font-bold text-outline block mb-0.5">TELÉFONO ACUDIENTE</label><input type="text" value={editingPatient.acudienteTelefono || ''} onChange={(e) => setEditingPatient({ ...editingPatient, acudienteTelefono: e.target.value })} className="w-full bg-surface-container-low border border-outline-variant/30 rounded-lg p-2.5 text-xs outline-none focus:ring-1 focus:ring-primary font-semibold" placeholder="310 456 7890" /></div>
                      <div><label className="text-[8px] font-bold text-outline block mb-0.5">PARENTESCO</label><select value={editingPatient.acudienteParentesco || ''} onChange={(e) => setEditingPatient({ ...editingPatient, acudienteParentesco: e.target.value })} className="w-full bg-white border border-outline-variant/30 rounded-lg p-2.5 text-xs outline-none font-bold text-primary"><option value="">Seleccionar...</option><option value="Hijo">Hijo/a</option><option value="Esposo">Esposo/a</option><option value="Hermano">Hermano/a</option><option value="Otro">Otro</option></select></div>
                    </div>
                  </div>
                </div>
